...
module Fractions =
    ...
    type Fraction =
        ...
        override x.ToString() =
            if (x.d = 1u) then sprintf "%d" x.n
            else sprintf "%d/%d" x.n x.d
        ...
        ///仮分数化
        static member Improper (f1:Fraction) (i:uint32) = { n = f1.d * i; d = f1.d }
        /// 分数の足し算引き算
        ...
        /// 足し算
        static member (+) (f1:Fraction, i:uint32) = f1 + (Fraction.Improper f1 i)
        /// 足し算
        static member (+) (i:uint32, f2:Fraction) = f2 + (Fraction.Improper f2 i)
        /// 引き算
        ...
        static member (/) (f1:Fraction, i:uint32) = f1 / (Fraction.Improper f1 i)
        /// 割り算
        static member (/) (i:uint32, f2:Fraction) = (Fraction.Improper f2 i) / f2
