...
/// 総当たり
let find () =
    // 探索対象の文字列を1次元配列とする。(nthをする場合よりメモリを使うが、配列の方が断然早い)
    let t = text.Replace("\r\n", " ").Trim()
    let cells = seq { for p in 0..3..t.Length -> int t.[p..p+1] } |> Seq.toArray
//    let cells = seq { for p in 0..3..t.Length -> int t.[p..p+1] } |> Seq.cache
    // 各方位の計算
    let rec findCells inc (x,y) (l : int list) =
        ...
        | (x,y) ->
            ...
            | [_;_;_;_;] ->
                ...
                ((x,y), l, num)
            | _ ->
                let cell = cells.[20 * y + x]
//                let cell = Seq.nth (20 * y + x) cells
